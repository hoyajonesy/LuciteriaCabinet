/**
 * Standalone Admin: Subscription Tier Management — list view.
 *
 * This is the /admin (AdminUser-authenticated) counterpart of
 * /app/admin/subscription-tiers. It reuses the exact same tier-admin service
 * and TierCard component, but is gated by requireAdmin() and lives inside the
 * standalone /admin console sidebar.
 *
 * Nested under the /admin layout (admin.jsx), which enforces the AdminUser
 * auth gate and renders the sidebar — so this route renders content only.
 */
...
export async function action({ request }) {
  ...
  // ActivityLog.userId is a FK to the User table; standalone admins live in
  // AdminUser, so we pass userId:null (audit logging is non-fatal and handles
  // null gracefully) and attribute the change via actorEmail.
  const userId = null;
...
export default function SubscriptionTiers() {
  ...
  const isChildActive = path !== "/admin/subscription-tiers";
  if (isChildActive) {
    return <Outlet />;
  }
  ...
  return (
    <div style={styles.wrap}>
      <div style={styles.header}>
        ...
        <Link to="/admin/subscription-tiers/new" style={styles.addBtn}>
          + Add New Tier
        </Link>
      </div>
      ...
      {tiers.length === 0 ? (
        <div style={styles.empty}>
          ...
          <Link to="/admin/subscription-tiers/new" style={styles.addBtn}>+ Add New Tier</Link>
        </div>
      ) : (
