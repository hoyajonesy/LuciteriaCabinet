/**
 * Public Wishlist View via ShareSlug — /w/:shareSlug
 * 
 * Shows a user's wishlist elements using a user-friendly shareSlug.
...
const getUserByShareSlug = async (shareSlug) => {
  const parts = shareSlug.split("-");
  if (parts.length < 2) return null;
  const idPrefix = parts[parts.length - 1];
  const namePrefix = parts.slice(0, parts.length - 1).join("");

  // Find all users whose ID starts with the suffix prefix
  ...
  // Find the exact match by matching the namePrefix
  return users.find((user) => {
    const userSlugPrefix = (user.firstName || "collector")
      .toLowerCase()
      .replace(/[^a-z0-9]/g, "");
    return userSlugPrefix === namePrefix;
  }) || users[0] || null;
};

export const loader = async ({ params }) => {
  const { shareSlug } = params;
  if (!shareSlug) throw new Response("Not Found", { status: 404 });

  const user = await getUserByShareSlug(shareSlug);
