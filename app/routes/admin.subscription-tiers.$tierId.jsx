/**
 * Standalone Admin: Subscription Tier — create / edit form.
 *
 * `/admin/subscription-tiers/new`      → create a new tier
 * `/admin/subscription-tiers/:tierId`  → edit an existing tier
 *
 * The /admin (AdminUser-authenticated) counterpart of the /app/admin tier form.
 * Reuses the same PricingFields / CollectionTypeSelector components and the
 * tier-admin service. Gated by requireAdmin().
 */
...
export async function action({ request, params }) {
  ...
  // ActivityLog.userId FKs to the User table; standalone admins live in
  // AdminUser, so pass userId:null and attribute via actorEmail.
  const userId = null;
  ...
  return redirect("/admin/subscription-tiers");
}
  ...
  return (
    <div style={styles.wrap}>
      <div style={styles.breadcrumb}>
        <Link to="/admin/subscription-tiers" style={styles.back}>← Subscription Tiers</Link>
      </div>
      ...
      <div style={styles.columns}>
        ...
        <Form method="post" style={styles.form} onSubmit={confirmIfNeeded}>
          ...
          />
          {/* Hidden input mirrors controlled state so it posts even if unchanged. */}
          ...
          <div style={styles.actions}>
            ...
            <Link to="/admin/subscription-tiers" style={styles.cancelBtn}>Cancel</Link>
          </div>
        </Form>
